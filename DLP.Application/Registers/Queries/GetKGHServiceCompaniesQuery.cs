...
public class GetKGHServiceCompaniesQuery : IOrderingQuery<Organization>, IRequest<OrdinalPaginatedList<KGHServiceCompanyDto>>
{
    ...
    private static readonly IReadOnlyDictionary<string, Expression<Func<Organization, object?>>> OrderingPropertyMappings =
        new Dictionary<string, Expression<Func<Organization, object?>>>(StringComparer)
        {
            ...
            { "nrOfCertifiedServiceTechnicians", x => x.Employees.Count(x => !x.IsDeleted && x.IsCertifiedTechnician) },
...
public class GetKGHServiceCompaniesQueryHandler : IRequestHandler<GetKGHServiceCompaniesQuery, OrdinalPaginatedList<KGHServiceCompanyDto>>
{
    ...
    public GetKGHServiceCompaniesQueryHandler(IAppDbContext context, IActivityLogger logger, IIdentityService identityService, ICurrentUserService currentUser)
    {
        _context = context;
        _logger = logger;
    ...
    public async Task<OrdinalPaginatedList<KGHServiceCompanyDto>> Handle(GetKGHServiceCompaniesQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            //        .Get(CustomPolicies.Registers.List.Name, CustomPolicies.Registers.ListRegistersOfKghCompanies.Name)
            //    : e => true;
            ...
            if (request.Type.HasValue && request.Type != OrganizationTypeEnum.INSTITUTION)
            {
                organizations = organizations.Where(x => x.Type == request.Type.Value);
            }

            if (request.BusinessActivityId.HasValue)
            {
                organizations = organizations.Where(x => x.BusinessActivityId == request.BusinessActivityId.Value);
            }

            OrdinalPaginatedList<KGHServiceCompanyDto> responseData;

            if (request.IsFromExport)
            {
                var response = organizations
                  .ApplyOrderByFunctions(request.GetOrderByFunction())
                  .ProjectToType<KGHServiceCompanyDto>().ToList();
                responseData = new OrdinalPaginatedList<KGHServiceCompanyDto>(response, response.Count, request.PageNumber, request.PageSize);
            ...
            else
            {
               ...
               .ApplyOrderByFunctions(request.GetOrderByFunction())
               .ProjectToType<KGHServiceCompanyDto>()
            ...
            await _logger.Add(new ActivityLogDto
            {
                ...
                Activity = "Successfully retrieved KGH Service Companies"
            });
        ...
        {
            await _logger.Exception(ex.Message, "Failed to retrieve KGH Service Companies", _currentUser.UserId);
            throw;
        }
    }
}
