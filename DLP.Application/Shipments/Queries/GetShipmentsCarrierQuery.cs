...
{
    public class GetShipmentsCarrierQuery : IOrderingQuery<Shipment>, IRequest<OrdinalPaginatedList<ShipmentsDto>>
    {
    ...
    public class GetShipmentsCarrierQueryHandler : IRequestHandler<GetShipmentsCarrierQuery, OrdinalPaginatedList<ShipmentsDto>>
    {
        ...
        public GetShipmentsCarrierQueryHandler(IAppDbContext context, ICurrentUserService currentUserService, IIdentityService identityService, IActivityLogger activityLogger)
        {
            _context = context;
        ...
        public async Task<OrdinalPaginatedList<ShipmentsDto>> Handle(GetShipmentsCarrierQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (!isAdmin)
                {
                    shipmentsList = shipmentsList.Where(u => u.CarrierOrganizationId == _currentUserService.OrganizationId);
                }
            ...
            {
                await _activityLogger.Exception(ex.Message, "An error occurred while handling the GetShipmentsCarrierQuery", _currentUserService.UserId);
                throw;
            }
        }
    }
