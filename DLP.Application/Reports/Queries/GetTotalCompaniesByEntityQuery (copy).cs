...
{
    public class GetServiceCompaniesByEntityQuery : IRequest<OrdinalPaginatedList<CompanyByEntityReportDto>>
    {
    ...
    public class GetServiceCompaniesByEntityQueryHandler : IRequestHandler<GetServiceCompaniesByEntityQuery, OrdinalPaginatedList<CompanyByEntityReportDto>>
    {
        private readonly IAppDbContext _context;
        private readonly IActivityLogger _logger;
        private readonly ICurrentUserService _currentUser;

        public GetServiceCompaniesByEntityQueryHandler(IAppDbContext context, IActivityLogger logger, ICurrentUserService currentUser)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<OrdinalPaginatedList<CompanyByEntityReportDto>> Handle(GetServiceCompaniesByEntityQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var query = from o in _context.Organizations
                            join m in _context.Municipalities on o.MunicipalityId equals m.Id
                            join ss in _context.CompanyRegisterTypes on o.Id equals ss.OrganizationId
                            join s in _context.StateEntities on m.StateEntityId equals s.Id
                            //where ss.Type == CompanyType.ServiceCompanyEnterpreneur
