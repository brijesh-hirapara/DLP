...
namespace DLP.Application.TransportManagemen.Jobs
{
    public class ChangeTransportCompletedOrRejectedCommand : IRequest<string>
    {
        public string Id { get; set; }
    }
    public class ChangeTransportCompletedOrRejectedCommandHandler : IRequestHandler<ChangeTransportCompletedOrRejectedCommand, string>
    {
        ...
        public ChangeTransportCompletedOrRejectedCommandHandler(IAppDbContext dbContext, IBackgroundJobClient backgroundJobClient, IActivityLogger activityLogger, IEmailCommunicationService emailCommunicationService, IHubContext<TransportHub> hubContext, ICurrentUserService currentUser)
        {
        ...
        public async Task<string> Handle(ChangeTransportCompletedOrRejectedCommand command, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var transportRequest = await _dbContext.TransportRequests
                    ...
                    .FirstOrDefaultAsync(x => x.Id == requestId && x.Status == TransportRequestStatus.UnderEvaluation, cancellationToken);
