...
const ExportButtonReportSubstancesPageHeader = ({ data, columns, title = "export_data" }) => {
  const { t } = useTranslation();

  const exportToExcel = () => {
      const table = document.getElementById('table-to-xls-substances');
    ...
    XLSX.utils.book_append_sheet(wb, ws, "import export substances report");

    // Save workbook
    XLSX.writeFile(wb, 'import-export-substances-report.xlsx');
  ...
  const exportToCSV = () => {
    ...
    // Save workbook
    XLSX.writeFile(wb, 'import-export-substances-report.csv');
  };

  const exportToPDF = () => {
    const element = document.getElementById('table-to-xls-substances'); // Replace 'table-to-xls' with the ID of your HTML element
    if (element) {
      const h3Element = document.createElement('h3');
      h3Element.innerText = 'MVTEO Annual Report on import/export of ozone depleting substance';
      const firstChild = element.firstChild;
      element.insertBefore(h3Element, firstChild);
      const title = 'MVTEO Annual Report on import/export of ozone depleting substance';
      html2canvas(element, {
        ...
        pdf.setProperties({
          title: 'MVTEO Annual Report on import/export of ozone depleting substance' // Set the title of the PDF document
        });
        pdf.save('import-export-substances-report.pdf');
      });
      h3Element.remove();
    }
  };
...
export { ExportButtonReportSubstancesPageHeader };
