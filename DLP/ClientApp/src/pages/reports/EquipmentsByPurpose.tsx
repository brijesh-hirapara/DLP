...
const EquipmentsByPurpose = () => {
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: t("reports:equipment-purpose", "Equipment Purpose"),
      dataIndex: "name",
      key: "name",
      sorter: true,
      sortDirections,
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: GuidReportResponseDto) => {
      const { id, name, stateEntityName, ordinalNumber, total } = item;
        ...
        <PageHeader
          ...
          buttons={[
            <ExportButtonPageApiHeader
              key="1"
              callFrom={'equipments-by-purpose'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""}
            />,
          ]}
        />
      </CardToolbox>
...
export default EquipmentsByPurpose;
