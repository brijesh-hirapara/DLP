...
const CompaniesByEntity = () => {
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsCompaniesByEntityGet({
      from: fromDate,
      to: toDate,
    });
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: any) => {
      const { id, name, total, type, ordinalNumber } = item;
    ...
    <>
      <CardToolbox>
        <PageHeader
          ghost
          title={t(
            "side-bar:reports.company-by-entity",
            "Equipment by Entity"
          )}
          buttons={[
            <ExportButtonPageApiHeader
            key="1"
            callFrom={'companies-by-entity'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""}
            />,
          ]}
        />
      </CardToolbox>
...
export default CompaniesByEntity;
