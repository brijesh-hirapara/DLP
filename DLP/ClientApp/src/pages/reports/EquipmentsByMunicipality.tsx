...
const EquipmentsByMunicilipality = () => {
  /**
   * Translation
   */
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsEquipmentsByMunicipalityGet({
      from: fromDate,
      to: toDate,
    });
    setData(data.items);
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: t("reports:municipality", "Municipality"),
      dataIndex: "name",
      key: "name",
      sorter: true,
      sortDirections,
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: GuidReportResponseDto) => {
      const { id, name, stateEntityId,stateEntityName, total, ordinalNumber } = item;
    ...
    <>
      <CardToolbox>
        <PageHeader
          ghost
          title={t(
            "side-bar:reports.equipments-by-entity",
            "Equipment by Entity"
          )}
          buttons={[
            <ExportButtonPageApiHeader
              key="1"
              callFrom={'equipments-by-municipality'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""}
            />,
          ]}
        />
      </CardToolbox>
...
export default EquipmentsByMunicilipality;
