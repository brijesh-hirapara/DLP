...
const ServiceCompaniesReport = () => {
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsServiceCompaniesGet({
      from: fromDate,
      to: toDate,
    });
      ...
      <CardToolbox>
        <PageHeader
          ...
          buttons={[
            ...
            key="1"
            callFrom={'service-companies'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""}
          />,
          ]}
        />
      </CardToolbox>
...
export default ServiceCompaniesReport;
