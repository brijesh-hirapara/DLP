...
const AnnualReportByServiceTechnician = () => {
  /**
   * Translation
   */
  ...
  const handleDeleteAnnualReportClick = async (id: string | undefined) => {
    try {
      if (!id) return;
      await reportsApi.apiAnnualReportByServiceTechnicianIdDelete({ id });
      openNotificationWithIcon(
        "success",
        t("annual-report-on-collected-substances:success-deleted", "Annual report on collected substances is deleted successfully!")
      );
      getData();
    } catch (err) {
      console.log({ err });
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsAnnualReportByServiceGet({
      ...request,
       ...modifiedSorting 
    });
    setData(data.items);
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: any) => {
      const { id, year,ordinalNumber, submitedDate, responsiblePerson, } = item;
      return {
        key: id,
        ...
        action: (
          ...
          >
            <Link to={`/reports/view-mvteo-report-by-service-technician/${id}`} title="View">
            ...
            <Button className="btn-icon" type="primary" to="#" shape="circle" >
              <Link to={`/reports/edit-report-by-service-technician/${id}`}>
            ...
            <>
              <Popconfirm
                title={t(
                  ...
                  "Are you sure delete this annual report on collected substances?"
                )}
              ...
              <Tooltip title={t("global.delete", "Delete")}>
                ...
                >
                  <FeatherIcon icon="trash-2" size={16} title="Delete"/>
                </Button>
                </Tooltip>
              </Popconfirm>
            </>
        ...
        <PageHeader
          ...
          buttons={[
                ...
                <Button
                  ...
                  key="add-button"
                >
                  <Link to="/reports/create-report-by-service-technician">
                    {t("add-new-report-on-collected-substances.button", "+ Add New Report On Collected Substances")}
                  </Link>
                </Button>
...
export default AnnualReportByServiceTechnician;
