...
import {
  GeneralReportDto,
  GeneralReportEntityDto,
} from "api/models/reports-general-dto";
import { useAuthorization } from "hooks/useAuthorization";
import moment from "moment";
import CustomDateRange from "components/customDatePicker/datePicker";
import { RefrigerantTypeDto } from "api/models";
...
const RefrigerantsByEntity = () => {
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsRefrigerantsByEntityGet({
      from: fromDate,
      to: toDate,
    });
    setData(data.items);
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      dataIndex: "stateEntityName",
      key: "stateEntityName",
      sorter: true,
      sortDirections,
    },
    {
      title: t("reports:refrigerant-by-entity", "Refrigerant Name"),
      dataIndex: "name",
      key: "name",
      sorter: true,
      sortDirections,
    ...
    {
      title: t("reports:quantity", "Quantity"),
      dataIndex: "quantity",
      key: "quantity",
      sorter: true,
      sortDirections,
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: any) => {
      const { id, name, quantity, stateEntityId,stateEntityName, ordinalNumber } = item;
    ...
    <>
      <CardToolbox>
        <PageHeader
          ghost
          title={t(
            "side-bar:reports.refrigerant-by-entity",
            "Refrigerant by Entity"
          )}
          buttons={[
            <ExportButtonPageApiHeader
            key="1"
            callFrom={'refrigerants-by-entity'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""}
          />,
...
export default RefrigerantsByEntity;
