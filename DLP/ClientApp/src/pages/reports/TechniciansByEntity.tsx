...
const TechniciansByEntity = () => {
  ...
  const getData = async () => {
    ...
    const { data } = await reportsApi.apiReportsTechniciansByEntityGet({
      from: fromDate,
      to: toDate,
    });
    setData(data.items);
  ...
  const generateTableData = () => {
    return (data ?? []).map((item: GuidReportResponseDto) => {
      const { id, name, ordinalNumber, stateEntityName, total } = item;
    ...
    <>
      <CardToolbox>
        <PageHeader
          ghost
          title={t(
            "side-bar:reports.technician-by-entity",
            "Technicians by Entity"
          )}
          buttons={[
            <ExportButtonPageApiHeader
              key="1"
              callFrom={'technicians-by-entity'} filterType={""} municipalityId={""} entityId={""} search={request.search} typeOfEquipmentId={""} from={query.from ? moment(query.from, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} to={query.to ? moment(query.to, "DD.MM.yyyy").format("MM/DD/yyyy") : ""} />
          ]}
        />
      </CardToolbox>
      <Main>
...
export default TechniciansByEntity;
